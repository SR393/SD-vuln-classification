...
def get_correlations(index, alphas, x_grid, emp_peak_widths_BL, emp_peak_widths_SD):
...
if __name__ == '__main__':
    ...
    if not os.path.exists('../data/logpca_primary_peak_width_correlation.json'):

        n_repeats = 1000

        corrs_func = partial(get_correlations, alphas = alphas, eps = eps, x_grid = x_grid,
        ...
        dp.dict_to_json(correlations_dict, f'../data/logpca_primary_peak_width_correlation.json')

    else:
        correlations_dict = dp.json_to_dict(f'../data/logpca_primary_peak_width_correlation.json')
    ...
    true_correlations = get_correlations(None, emp_peak_locs_BL = emp_peak_locs_BL, emp_peak_locs_SD = emp_peak_locs_SD, 
